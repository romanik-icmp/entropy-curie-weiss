...
When the ideal-gas contribution to the chemical potential, $\mu_{\rm{id}} = k_{\rm{B}}T \ln(N\Lambda^3/V)$, is taken into account, it becomes evident that the first term in~\eqref{eq:entropy1} represents the entropy of a system of noninteracting molecules arranged on a lattice, subject to the strict single-occupancy constraint (one particle per cell), see~\citep[(47.4)]{Hill56}.
...
The dependence of the entropy per cell on the chemical potential is illustrated in Figure~\ref{fig:entropy3}c, and that on the density in Figure~\ref{fig:entropy3}d.
...
That of the entropy per particle on density is illustrated in \citep[Fig.~8]{dLBKGS11}. All Figures contain curves for multiple temperature values, and different Hamiltonian parameters of the Hubbard model.
The dependence of the entropy per particle on the chemical potential is absent though.

In~\citep{PKvHT08} the entropy in the single-band Bose-Hubbard model is studied in one and two dimensions by the quantum Monte Carlo methods. The entropy per site as a function of the filling is presented in \citep[Fig.~2]{PKvHT08} for the homoheneous 1D Bose-Hubbard system.




Dependence of entropy on the number density (filling number) can be found in the following studies \citep{PKvHT08,dLBKGS11,PKF20}, and our results can be qualitatively compared with the ones from these references.

The dependence of the entropy on temperature is presented in \citep{CPS07} for the three-dimensional Bose-Hubbard model, in \citep{PKvHT08} for the one-dimensional Bose-Hubbard system.
...
\section{Ideas}
\begin{itemize}
	\item Ideal lattice gas contribution is explicitly singled out;
	...
	\item The requirement to place more and more particles into one cell creates a diverging negative entropy.
	See similar statement in~\citep[Sec.~IV]{Stillinger76}
\end{itemize}
